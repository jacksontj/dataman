...
func TestJsonStreams(t *testing.T) {
	...
	for i, test := range tests {
		f := func() (ServerStream, ClientStream) {
			...
			server := NewJSONServerStream(context.Background(), test.chunkSize, test.flushInterval, writer)
			client := NewJSONClientStream(reader)
